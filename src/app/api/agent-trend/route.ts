...
`;
    ...
    if (style === 'blog2') {
      prompt = `
당신은 대한민국 상위 0.1% 네이버 블로그 트래픽 마스터(SEO 전문가)이자, **'60대 및 비즈니스/경제 특화 메가 블로그'**를 운영하는 총괄 편집장입니다.
현재 이 블로그(두 번째 블로그)는 일 방문자 500명 수준이나, 이를 폭발적으로 성장시키기 위해 철저히 **[크리에이터 어드바이저 데이터]**와 **[60대/경제 마이크로 타겟팅]**에 기반한 틈새 황금 롱테일 키워드 5개를 발굴해야 합니다.
${bannedSection}
${feedbackLearningGuidance}
...
[🚨 두 번째 블로그(Blog2) 핵심 미션: 60대/경제 특화 마이크로 딥다이브 추론 🚨]
- 방향성: 대중적인 일반 지식은 버리고, 구매력 있는 60대와 재테크/투자자들이 미친듯이 파고드는 **'깊고 좁은 뾰족한 정보'**를 타겟팅합니다.
- 능동적 추론: 제공된 일주일 치 빅데이터 카테고리 흐름을 읽고, **오늘(현재 날짜) 또는 앞으로 며칠 내에 새롭게 트래픽이 터질 '예측 롱테일 키워드'**를 추론하세요. 단순히 과거 데이터를 앵무새처럼 반복하면 절대 안 됩니다!
- 단, 상상으로 지어내는 것은 금지입니다. 추론된 아이디어를 반드시 구글 검색을 통해 '최근 실시간으로 강력한 트래픽이 올락내리락 하는지' 검증하고 최종 5개를 엄선하세요.
- 포괄적인 키워드는 무조건 버리고, 60대와 경제 타겟이 클릭할 수밖에 없는 자극적이고 실질적인 롱테일 5개를 추출하세요.
