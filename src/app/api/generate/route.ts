...
1. 반드시 글의 맨 최상단(도입부)에는 이 이슈가 시장에 미치는 [핵심 영향력 요약(증시 향방, 환율, 관련주 등)]을 시각적으로 가장 먼저, 누구나 한눈에 알기 쉽게 강조하여 배치하세요. (예: "결과: S&P500 영향도 - 매우 높음, 관련 자산: 비트코인 상승예상")
...
`;
    ...
    if (category === 'sports') {
      try {
        const aiImgRes = await ai.models.generateImages({
          model: 'imagen-3.0-generate-002',
          prompt: `A high quality cel-shaded 2D vector graphic illustration of a professional sports/baseball player representing the keyword "${keyword}". Intense and dynamic expression, thick outlines, clean sticker cartoon style maskot. Full color masterpiece, centered, pure white background.`,
          config: { numberOfImages: 1, outputMimeType: 'image/jpeg', aspectRatio: '1:1' }
        });
        const base64Bytes = aiImgRes.generatedImages?.[0]?.image?.imageBytes;
        if (base64Bytes) {
          thumbnailHtml = `<div style="text-align: center; margin-bottom: 24px;">
            <img src="data:image/jpeg;base64,${base64Bytes}" alt="${keyword} 특화 썸네일" style="max-width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05);" />
          </div>`;
        }
      } catch (imgErr) {
        console.error("Imagen API failed for sports thumbnail:", imgErr);
        // 실패 시 아래 fallback으로 넘어감
      }
    } else if (category === 'finance') {
      try {
        const aiImgRes = await ai.models.generateImages({
          model: 'imagen-3.0-generate-002',
          prompt: `A high quality cel-shaded 2D vector graphic caricature illustration of a prominent global political or economic figure (like Donald Trump, Jerome Powell, Elon Musk, etc.) or a stressed Wall Street trader, strongly representing the financial/economic keyword "${keyword}". Intense and dynamic expression, holding a chart or money, thick outlines, clean sticker cartoon style maskot. Full color masterpiece, centered, pure white background.`,
          config: { numberOfImages: 1, outputMimeType: 'image/jpeg', aspectRatio: '1:1' }
        });
        const base64Bytes = aiImgRes.generatedImages?.[0]?.image?.imageBytes;
        if (base64Bytes) {
        ...
        console.error("Imagen API failed for finance thumbnail:", imgErr);
        // 실패 시 아래 fallback으로 넘어감
      }
    }
