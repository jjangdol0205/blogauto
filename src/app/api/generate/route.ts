...
export async function POST(req: Request) {
  try {
    ...
    특히, 추상적인 개념(주식, 투자, 정책, AI 등)일 경우 사무실(office)이나 사람(people) 같은 밋밋한 사진이 나오지 않도록, '그래프(graph)', '차트(chart)', '돈(money)', '증권(stock market)', '미래도시(futuristic city)' 등 시각적으로 직관적이고 상징적인 사물/풍경 단어를 선택하세요.
...
[작성 가이드(매우 중요 - ★독자 친화적이고 매력적인 원고 타겟★)]

1. 분량과 깊이 (가독성을 잃지 않는 선에서 풍성하게):
   - **글자 수:** 공백 제외 최소 1,500자 ~ 2,000자 이상 아주 상세하게 작성하되, 지루하지 않게 작성하세요.
   - **쉽고 매력적인 전달력 (가장 중요):** 너무 현학적인 단어나 딱딱한 전문 용어, 논문 같은 문투는 절대 피하세요! 중학생이 읽어도 단번에 이해될 만큼 쉬운 단어와 친근한 비유를 적극 사용하세요. (예: "본질적 가치 추구" -> "우리가 진짜 이것을 찾는 이유") 독자에게 직접 이야기하듯 편안하고 대화하는 듯한 친근감 있는 말투를 유지하면서도 내용의 깊이를 챙기는 것이 핵심입니다.
   - **구조의 미학:** 밋밋한 줄글 대신, 여러 개의 <hr>(구분선), <blockquote>(핵심 요약), 번호 매기기, 그리고 <table>(한눈에 쏙 들어오는 깔끔한 비교표) 등을 적극 활용하여 시각적으로도 가볍고 아름다운 문서를 만드세요.

2. 클릭을 유도하되 호감 가는 직관적인 제목 (Title):
   - **핵심 키워드 배치:** 타겟 키워드는 제목에 자연스럽게 녹이되 상위 노출을 고려하세요.
   - **직관성과 호기심 자극:** 너무 거창한 제목보다는 "OOO, 모르면 손해보는 3가지 꿀팁", "초보자도 1분 만에 이해하는 OOO 완벽 정리" 등 독자가 당장 클릭하고 싶어지는 친근하면서도 확실한 혜택을 주는 타이틀을 구성하세요.
