...
`;
    ...
    try {
      ...
      const shortTitle = parsedResult.title.length > 20 ? parsedResult.title.substring(0, 20) + "..." : parsedResult.title;
      
      const imgRes = await ai.models.generateImages({
        model: 'imagen-4.0-fast-generate-001',
        prompt: `유튜브 썸네일 스타일, 화면 한가운데에 "${shortTitle}" 이라는 굵고 크고 화려한 한국어 타이포그래피 글씨 배치, 배경은 고퀄리티 2D 웹툰 일러스트, 밝고 쨍한 색감, 관련 소품 배치, 글자가 아주 선명하게 보여야 함.`,
        config: { 
          numberOfImages: 1, 
          aspectRatio: '16:9',
